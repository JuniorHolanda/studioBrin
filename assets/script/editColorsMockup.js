...
//lista de cores
const listBtnColor = ["vermelho", "royal", "amarelo", "roxo", "preto", "vinho", "lilas"];
...
export function createBtnColor(imagem) {
  ...
  function changeColorPartProduct(color) {
    ...
    for (let i = 0; i < listImgPart.length; i++) {
      ...
      if (nameProduct.includes(imagem)) {
        listImgPart[i].setAttribute('data-change-color' , color)
      } // fim do if
    } // fim do for
  ...
  for (let i = 0; i < listBtnColor.length; i++) {
    ...
    btnColor.addEventListener("click", () =>
      changeColorPartProduct(listBtnColor[i])
    );
  }
